                        ...
                        <div class="header">
                           ...
                           Arquivo de Pessoas
                            ...
                            include('../controller/db_functions.php');
                             $pdo = conectar(); 
                             $busca_rapida = $pdo->prepare("SELECT strFicha from pessoa order by strFicha DESC limit 1");
                             $busca_rapida->execute();
                             $k = $busca_rapida->fetch(PDO::FETCH_ASSOC);
                             $dt_hoje = date('Y-m-d');

                             $busca_count = $pdo->prepare("SELECT strFicha FROM pessoa where dataCadastro >= '$dt_hoje'");
                             $busca_count->execute();
                             #$var_count = $busca_count->fetch(PDO::FETCH_OBJ);

                             $conta_hoje = $busca_count->rowCount();
                             


                             ?>
                            <span class="badge bg-indigo">ULTIMA FICHA CADASTRADA: <?=$k['strFicha']?> </span><span style="margin-left: 10px" class="badge bg-indigo"> QUANTIDADE DE CADASTROS FEITOS HOJE: <?=$conta_hoje?> </span><!--span style="margin-left: 10px" class="badge bg-indigo"> ONTEM:</span-->
...
function showCustomer(nome,cpf) {
  ...
  xhttp.open("POST", "busca-rapida-pessoas.php?nome="+nome, true);
  xhttp.send();
}
</script>
...
function showCustomer2(nome,cpf) {
  ...
  xhttp.open("POST", "busca-rapida-pessoas.php?cpf="+nome, true);
  xhttp.send();
}
</script>
...
function showCustomer3(nome,cpf) {
  ...
  xhttp.open("POST", "busca-rapida-pessoas.php?razaoSocial="+nome, true);
  xhttp.send();
}
</script>
...
function showCustomer12(nome,cpf) {
  ...
  xhttp.open("POST", "busca-rapida-pessoas.php?numficha="+nome, true);
  xhttp.send();
}
</script>
                ...
                <div class="modal-dialog modal-sm" role="document">
                    <div class="modal-content bg-grey">
                     <div class="alert bg-grey alert-dismissible" role="alert">
                            ...
                            <form method="GET" id="formcartao" action="PDFS/pdf-pessoa-fisica-a6.php?id=document.getElementById('idselofsc').value">
                    ...
                    <a onclick="window.open('PDFS/pdf-pessoa-fisica-a6.php?reimpressao=S&id='+document.getElementById('idselofsc').value)" id="botaoreimpressao" class="btn waves-effect bg-red col-sm-12"><i class="material-icons">print</i> REIMPRIMIR CARTÃO</a>
                    <br><br><br>
