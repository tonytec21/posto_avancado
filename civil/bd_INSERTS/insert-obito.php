...
#executando limpeza
/*
$busca_limpeza = $pdo->prepare("SELECT * FROM registro_obito_novo");
$busca_limpeza->execute();
$limpa = $busca_limpeza->fetchAll(PDO::FETCH_OBJ);
foreach ($limpa as $l) {
if ($l->SELO == '' && $l->NOME =='' && $l->NOMEMAE == '' && $l->NOMEPAI == '' && $l->LIVROOBITO =='' && $l->FOLHAOBITO =='' ) {
$del  = $pdo->prepare("DELETE FROM registro_obito_novo where ID = '$l->ID'");
$del->execute();
}
}
*/
...
header('location:../index/registro-obito-novo.php?acervo=ok&id='.$id);	
}	
else{
header('location:../index/registro-obito-novo.php?id='.$id);
}
