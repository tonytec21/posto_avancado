...
if (isset($_POST['dados_add'])) {
	...
	$busca_table_add = $pdo->prepare("SELECT * FROM info_registros_civil where id_registro_obito = '$id'");
	$busca_table_add->execute();
	
	if ($busca_table_add ->rowCount()<1) {
	$in_table_add = $pdo->prepare("INSERT INTO `bookc`.`info_registros_civil` (`id_registro_obito`, `inteiro_teor`, `observacoes_registro`, `obs_visivel_certidao`) VALUES ('$id', '$int_teor', '$obs', '$exibir_obs_registro');");
	$in_table_add->execute();
	}

	$query = "UPDATE info_registros_civil set obs_visivel_certidao = '$exibir_obs_registro', observacoes_registro ='$obs', inteiro_teor = '$int_teor' where id_registro_obito = '$id'" ;
