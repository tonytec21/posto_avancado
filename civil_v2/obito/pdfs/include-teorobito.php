			...
			<?php if (isset($id_pessoa)): ?>
			 ...
			 <!--COR-->
			 <?php if (!empty($dados['COR'])): ?>
			 	, de cor <?=retornacorpele($dados['COR'])?>, 
			 <?php endif ?>
			...
			<?php if (!empty($dados['NOMEPAI'])): ?>
				...
				if (isset($id_pai)) {
					if (empty($dados['CPFPAI'])){
						echo montaqualificacaotext_id($id_pai, $dataRegistro,'');
						 echo " e de ";
					}
					else{
						echo montaqualificacaotext_cpf($dados['CPFPAI'], $dataRegistro,'');
						 echo " e de ";
					}
				}
				else{
					  echo montaqualificacaocivil($dados,"PAI", "", $dataRegistro);
					   echo " e de ";
				}
				?>
			<?php endif ?>
			...
			<?php endif ?>
				...
				<?php if (empty($dados['DATANASCIMENTOPAI']) && empty($dados['DATANASCIMENTOPAI']) ): ?>
				.
				<?php endif ?>
			...
			$obs = $pdo->prepare("SELECT * FROM info_registros_civil where id_registro_obito = '$id'");
			$obs->execute();
