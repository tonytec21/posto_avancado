<div id="containerselos" style="width: 100%;max-width: 100%;display: flex;flex-wrap: wrap;flex-direction: row;padding-top:5px">
	...
	for ($i=1; $i <= $_POST['qtdatos'] ; $i++):?>
		<?php if (isset($_POST['SELO'.$i]) && $contselo <=3): $contselo++; ?>
			...
			<?php 
			$dataav = date('Y-m-d');
			$averbacao = $pdo->prepare("SELECT * FROM averbacoes_civil where strLivro = '$livro' and strFolha = '$folha' and strMotivoAverbacao!='0' and strTipo = 'OB' and dataAverbacao = '$dataav'");
			$averbacao->execute();
