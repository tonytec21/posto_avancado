...
<!-- (???) MÃE DE MENOR QUALIFICAÇÃO/OBSERVAÇÃO (???)
			<?php if ($r->QUALIDADEDECLARANTE == 'MAEDEMENOR'): $repmae = explode(",", $r->ROGODECLARANTE);?>
			<br>
			<strong><?="Observação: A mãe é de menor de idade, estando acompanhado do sr(a) ".$repmae[0]?>
			<?="sendo o(a) mesmo(a) seu(ua) ".$repmae[1].". Que assina este termo."?></strong>
			<?php endif ?><br><br>

			<?php if ($r->QUALIDADEDECLARANTE == 'MAEDEMENOR'): ?>
			<p style="max-width:  100%; text-align: center;">__________________________________________________ <br> 
			<?=mb_strtoupper($repmae[0])?></p><br>
			<?php endif ?> 
-->	
			...
			<?php if (isset($r->NOMEDECLARANTE) && !empty($r->NOMEDECLARANTE) && strlen($r->NOMEDECLARANTE)>3): ?>
			<br><br>
			<p style="max-width:  100%; text-align: center;">__________________________________________________ <br> 
			<?=mb_strtoupper($r->NOMEDECLARANTE)?></p>
			<?php endif; ?>
			<?php endif ?>
			...
			<!-- Qualificação - Rogo do declarante -->
		 	<p style="display: inline-block;width: 40%; text-align:justify;font-weight: bold">
			<!-- Nome do rogo do declarante -->
			<?="O declarante, na impossibilidade de assinar, está acompanhado de seu assinante a rogo, ".mb_strtoupper($b->strNome)?>, 
			
			<!-- Nacionalidade do rogo do declarante -->
			<?php if (!empty($b->strNacionalidade)): ?>
			<?=$b->strNacionalidade?>, 	
			<?php endif ?>

			<!-- Estado civil do rogo do declarante -->
			...
			<!-- Profissão do rogo do declarante -->
			<?php if (!empty($b->strProfissao)): ?>
			<?=mb_strtolower($b->strProfissao)?>, 	
			<?php endif ?>
			
			<!-- Naturalidade do rogo do declarante -->
			<?php if (!empty($b->strNaturalidade)): ?>
			<?php  $c = PESQUISA_ALL_ID('uf_cidade',intval($b->strNaturalidade)); foreach($c as $c):?>
			...
			<?php endif ?>
			<!-- Endereço do rogo do declarante -->
			<?="residente e domiciliado(a) à ".mb_convert_case($b->strLogradouro, MB_CASE_TITLE, "UTF-8")?>,
			<?php if ($b->strBairro!=''): ?>
			<?=mb_convert_case($b->strBairro, MB_CASE_TITLE, "UTF-8")?>,
			<?php endif ?>
			<?php  $c = PESQUISA_ALL_ID('uf_cidade',intval($b->intUFcidade)); foreach($c as $c):?>
			<?=mb_convert_case($c->cidade, MB_CASE_TITLE, "UTF-8")."/".$c->uf.". Que assina este termo."?>
			<?php endforeach ?>
			</p>

		 	<table style="border: none; display: inline-block; margin-left: 20px; width: 51%; margin-bottom: -30px">
				...
				<tr style="border: none">
					<td style="border: none;text-align: center;"><?=strtoupper($r->NOMEDECLARANTE)?> </td>
					<td style="border: none; text-align: center; "><p style="margin-top: -40px"><?=strtoupper($b->strNome)?>
					<br>ASSINANTE A ROGO</p></td>
				</tr>
			</table>
			...
			<?php if ($r->NOMETESTEMUNHA1 != '' && $r->NOMETESTEMUNHA1 != 'NAO_DECLARADO' && $r->NOMETESTEMUNHA1 !='NULL'): ?>
				<p style="max-width:  100%; text-align: center;">________________________________________<br>
				<?=$r->NOMETESTEMUNHA1?><br>TESTEMUNHA</p>
			<?php endif ?>

			<!-- ASSINATURA TESTEMUNHA 2 -->
			<?php if ($r->NOMETESTEMUNHA2 != '' && $r->NOMETESTEMUNHA2 != 'NAO_DECLARADO' && $r->NOMETESTEMUNHA2 !='NULL'): ?>
				<p style="max-width:  100%; text-align: center;">________________________________________<br>
				<?=$r->NOMETESTEMUNHA2?><br>TESTEMUNHA</p>
			<?php endif ?>

			<!-- ASSINATURA DO ESCREVENTE -->
			<br>
			<p style="max-width:  100%; text-align: center;">________________________________________<br>
				<?=mb_strtoupper($nome_assinatura)?>
				<br>
				<?=mb_strtoupper($funcao_assinatura)?>
			</p> 
