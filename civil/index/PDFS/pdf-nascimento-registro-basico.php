                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header row">
                           ...
                           <h2 class="col-md-7">REGISTRO NASCIMENTO LIVRO A</h2>
                           ...
                           <a href="pdf-nascimento-registro-basico?id=<?=$id?>&id_apagar=<?=$id?>" class="btn col-md-3 waves-effect bg-red "><i class="material-icons">stop_screen_share</i> Limpar edições feitas </a>
                        </div>
                        <div class="body">
...
if (isset($_GET['v2'])) {
include('config_paginas/registro-nascimento-registro-basicov2.php');
}
elseif (isset($_GET['v3'])) {
include('config_paginas/registro-nascimento-registro-basicov3.php');
}
else{
include('config_paginas/registro-nascimento-registro-basico.php');  
}
...
$up = $pdo->prepare("update salvar_editor set TEXTO = '$texto_salvar' where IDREGISTRO = '$id' and TIPO = 'TERMO_NASCIMENTO_PADRAO' ");
$up->execute();
}
else{
$in = $pdo->prepare("INSERT into salvar_editor values('$id', 'TERMO_NASCIMENTO_PADRAO', '$texto_salvar')");
$in->execute();
}
...
$_SESSION['sucesso'] = 'ALTERAÇÕES SALVAS';
 ...
 ?>
  ...
  <script type="text/javascript">
    window.location.assign('pdf-nascimento-registro-basico?id=<?=$id?>');
   ...
   <script type="text/javascript">
    ...
    $('#versaotermo').change(function(){
      if ($('#versaotermo').val() == '1') {
        window.location='pdf-nascimento-registro-basico.php?id=<?=$id?>';
      }
      else if ($(this).val() == '2') {
          window.location='pdf-nascimento-registro-basico.php?id=<?=$id?>&v2=ok';
        }

      else if ($(this).val() == '3') {
          window.location='pdf-nascimento-registro-basico.php?id=<?=$id?>&v3=ok';
        }
    });
