...
include('../../selador/civil_geral.php');
      ...
      #PROXIMO PASSO ENVIANDO A SOLICITAÇÃO DO SELO: ===============================================================================================
                  ...
                  $nomeparte1 =tiraaspas($r->NOMENUBENTE1);
                  $docparte1=$r->CPFNUBENTE1;
                  $nomeparte2 =tiraaspas($r->NOMENUBENTE2);
                  $docparte2=$r->CPFNUBENTE2;
                  $nomeparte3 ='';
                  $docparte3='';
                  $nomeparte4 ='';
                  $docparte4='';
...
if (isset($_GET['motivoatoisento']) && $_GET['motivoatoisento']!='') {
                ...
                $insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','INT_TEOR_CASAMENTO','2','1','$SELO',CURRENT_TIMESTAMP,'CER','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNO')");
      ...
      if (isset($_GET['selobusca']) && $_GET['selobusca']!='') {
                            ...
                            $NUMEROPAPEL = $_GET['NUMEROPAPELSEGURANCA'];  
                            $funcionario = $_SESSION['nome'];
                            $insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','BUSCA_INT_TEOR_CASAMENTO','2','1','$SELOBUSCA',CURRENT_TIMESTAMP,'CER','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNOBUSCA')");
                            $UPSEGURANCA = $pdo->prepare("UPDATE folhaseguranca set STATUS = 'U' WHERE EMPRESA = '$TIPOPAPEL' AND PAPEL = '$NUMEROPAPEL'");
                            $UPSEGURANCA->execute();  
...
$busca_ja_existe = $pdo->prepare("SELECT * FROM salvar_editor where IDREGISTRO = '$id' and TIPO = 'INT_TEOR_CASAMENTO'");
  ...
  <?php if ($busca_ja_existe->rowCount()==0){
    ...
    if (isset($SELO)) {include('config_paginas/registro-casamento-inteiro-teor.php');} }?>
