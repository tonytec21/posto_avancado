  ...
  $.post('control/busca_dados_registro.php', {
    ...
    function(data) {  
            ...
            if (typeof retorno['erro'] !== "undefined") {
             ...
             if ($('#LIVROOBITO').val()=='' && $('#FOLHAOBITO').val()=='' && $('#TERMOOBITO').val()=='') {
...
$.post('../selador/gerar-selo.php', {
  ...
  'natureza_solicitacao' : "REGISTRO_OBITO"
...
function(data) {  
  ...
  if (typeof retorno['status'] !== "undefined") {
   ...
   console.log(retorno);
   campos_input('ATOOBITO', idregistro);
...
function pesquisaregistro(){
  ...
  var LIVROOBITO = $('#LIVROOBITO').val();
  var FOLHAOBITO = $('#FOLHAOBITO').val();
  var TERMOOBITO = $('#TERMOOBITO').val();
    ...
    ) 
  {
    ...
    xhttp.open("POST", "control/busca-pesquisa-obito.php?NOME="+NOME+"&NOMEMAE="+NOMEMAE+"&LIVROOBITO="+LIVROOBITO+"&FOLHAOBITO="+FOLHAOBITO+"&TERMOOBITO="+TERMOOBITO+"&MATRICULA="+MATRICULA+"&DATAENTRADA="+DATAENTRADA, true);
    xhttp.send();
...
function previewreg(id){
  ...
  xhttp.open("POST", "pdfs/preview-obito-livro.php?id="+id, true);
  xhttp.send();
}
