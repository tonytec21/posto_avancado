...
header('location:../notas-lavratura-rcto.php?conf1=ok&id='.$id);
...
if (isset($_POST['setTipoTestamento'])) {
$setTipoTestamento = $_POST['setTipoTestamento'];
}else {
	$setTipoTestamento = NULL;

}
if (isset($_POST['dataTestamento'])) {
$dataTestamento = $_POST['dataTestamento'];
}else {
	$dataTestamento = NULL;

}
$dataTestamento= date('Y/m/d', strtotime($dataTestamento));

if (isset($_POST['strTestamentoObservacoes'])) {
$strTestamentoObservacoes = $_POST['strTestamentoObservacoes'];
}else {
	$strTestamentoObservacoes = NULL;

}
UPDATE_CAMPO_ID('notas_lavratura','setTipodeAtoCep',1,$id);
UPDATE_CAMPO_ID('notas_lavratura','setTipoTestamento',$setTipoTestamento,$id);
UPDATE_CAMPO_ID('notas_lavratura','dataTestamento',$dataTestamento,$id);
UPDATE_CAMPO_ID('notas_lavratura','strTestamentoObservacoes',$strTestamentoObservacoes,$id);
...
$_SESSION['sucesso'] = 'SUCESSO CONTINUE O PREENCHIMENTO DO FORMULÁRIO';
header('location:../notas-lavratura-rcto.php?conf4=ok&id='.$id);
...
#Quando for cep:
$pesquisa_num_ordem = $pdo->prepare("SELECT strFolha,strFolhaFinal,strFolhaComplemento,setTipodeAtoCep,strSelo,Ordem, COUNT(Ordem) AS Quantidade FROM `notas_lavratura` WHERE strFolha > 0 and setTipodeAtoCep = '1'  and (strSelo IS NOT NULL || strSelo != '') ORDER BY `notas_lavratura`.`strFolha`");
...
$pesquisa_num_ato = $pdo->prepare("SELECT strFolha,strFolhaFinal,strFolhaComplemento,setTipodeAtoCep,strSelo,numAto, COUNT(numAto) AS Quantidade FROM `notas_lavratura` WHERE strFolha > 0 and setTipodeAtoCep = '1'  and (strSelo IS NOT NULL || strSelo != '') ORDER BY `notas_lavratura`.`strFolha`");
...
header('location:../notas-lavratura-rcto.php?conf3=ok&id='.$id);
