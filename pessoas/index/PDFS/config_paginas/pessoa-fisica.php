...
$_SESSION['tabelavigente'] = $tabela_custas["notas"];
...
<body>
	...
	echo 'NENHUM SELO FOI GERADO, NÃO SERÁ POSSIVEL A REIMPRESSÃO';
	break;
	}
...
<?php if (isset($key->hiddencaminhofoto) && !empty($key->hiddencaminhofoto) && $key->hiddencaminhofoto != 'NULL'): ?>
		...
		<img style="width: 50px; height: 70px;" src="../<?=$key->hiddencaminhofoto?>">
		</div>
<?php else: ?>
...
<fieldset style="height:2px;width: 20%; display: inline-block;font-size:8px"><legend>NATURALIDADE</legend>
	...
	<?php if (intval($key->strNaturalidade) == '5300109'):
	$cid_ext = explode("/", $key->strNaturalidade);
	echo $cid_ext[3];	
...
<fieldset style="height:2px;width: 20%; display: inline-block;font-size:8px"><legend>CIDADE</legend><div style="margin-top:-3px;">
	...
	<?php if (intval($key->intUFcidade) == '5300109'):
	$cid_ext = explode("/", $key->intUFcidade);
	echo $cid_ext[3];	
...
<fieldset style="height:2px;width: 20%; display: inline-block;font-size:9px"><legend>EMAIL</legend> <?=strtoupper($key->strEmail)?></fieldset>
</div>
