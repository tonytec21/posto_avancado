...
$pesquisa_apresentante = $pdo->prepare("SELECT * FROM portador_automatizado where codigo = '$b->numero_codigo_portador_transacao'");
 ...
 if ($token !='') {
      ...
      #SELO1 - 17.4...: ===============================================================================================
							...
							$insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','PAGAMENTO_PROTESTO','5','1','$SELO',CURRENT_TIMESTAMP,'GER','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNO')");
	  ...
	  #SELO - 17.5.1...: ===============================================================================================
							...
							$NUMEROPAPEL = '0';  
							$funcionario = $_SESSION['nome'];
							$insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','CERTIDAO_LIQUIDACAO_PROTESTO','5','1','$SELO',CURRENT_TIMESTAMP,'GER','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNO')");
  ...
  $SELO = '0';
 	...
 	?>
					...
					<div style="border: 1px solid black; border-radius: 4px; margin-right: 1cm; margin-left: 1cm; display: block;">
						...
						<p style="margin-left: 10px; text-align: center;">Recebi de <?=mb_strtoupper($b->nome_devedor_transacao)?> , a quantia abaixo descrita <br> concernente ao título acima especificado, acrescida das custas e Certidão de liquidação. </p>
					...
					<div style="border: 1px solid black; border-radius: 4px; margin-right: 1cm; margin-left: 1cm; display: block;">
						...
						<p style="text-align: center">Certifico e dou fé, a requerimento feito por pessoa interessada, que o título nº <?=$b->numero_titulo_transacao?> , no valor de
						R$: <?=number_format($b->saldo_titulo_transacao,2,",",".")?> apresentado por <?php $pesquisa_apresentante = $pdo->prepare("SELECT * FROM protesto_automatico_header where numero_sequencial_remessa_header = '$b->id_header'");
						$pesquisa_apresentante->execute();
						$l = $pesquisa_apresentante->fetchAll(PDO::FETCH_OBJ);
						foreach ($l as $l) {
						$apresentante = $l->nome_portador_header;
						}
						?>
						<b><?=mb_strtoupper($apresentante)?></b> , FOI LIQUIDADO nesta data</p> <br>
						<p style="text-align: center">
										<?php $h = PESQUISA_ALL('cadastroserventia'); foreach ($h as $h): 
										$u = PESQUISA_ALL_ID('uf_cidade',$h->intUFcidade);
										foreach ($u as $u):?>	
					...
					<?php endif ?></p>
										<br>
										<b> Valor Certidão: </b> R$ <?=$valor_ato?>;
										<b>Código Tabela: </b> 17.5.1 <br><br>
					...
					echo '<img style="max-width:70%; margin-left:40px;" src="'.$nome.'" />';
					?>
...
<?php 
 				$busca_livro = $pdo->prepare("SELECT * FROM livro_protestos_pagamentos where assento = '$b->numero_protocolo_cartorio_transacao'");
 				 ...
 				 ?>
 				<h3 style="font-weight: bold; margin-top: -50px;">Pagamento registrado no Livro <?=$livro_titulo?> folha: <?=$folha_titulo?>, Protocolo nº <?=$b->numero_protocolo_cartorio_transacao?></h3>
...
if ($pagamento_diferido == 'sim') { 					
      ...
      #SELO1 - 17.4...: ===============================================================================================
							...
							$NUMEROPAPEL = '0';  
							$funcionario = $_SESSION['nome'];
							$insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','PAGAMENTO_PROTESTO','5','1','$SELO',CURRENT_TIMESTAMP,'CPL','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNO')");
							$insert_auditoria->execute();
	  ...
	  #SELO - 17.9...: ===============================================================================================
							...
							$NUMEROPAPEL = '0';  
							$funcionario = $_SESSION['nome'];
							$insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','ARQUIVAMENTO','5','1','$SELO',CURRENT_TIMESTAMP,'CPL','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNO2')");
							$insert_auditoria->execute();
	  ...
	  #SELO - CONDUÇÃO...: ===============================================================================================
							...
							$NUMEROPAPEL = '0';  
							$funcionario = $_SESSION['nome'];
							$insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','CONDUCAO','5','1','$SELO',CURRENT_TIMESTAMP,'CPL','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNO4')");
							$insert_auditoria->execute();
	...
	if (!isset($_GET['semcert'])) {
	  #SELO - 17.5.1...: ===============================================================================================
							...
							$NUMEROPAPEL = '0';  
							$funcionario = $_SESSION['nome'];
							$insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','CERTIDAO_LIQUIDACAO_PROTESTO','5','1','$SELO',CURRENT_TIMESTAMP,'CPL','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNO3')");
							$insert_auditoria->execute();
	...
	if ($b->data_edital!=''){
      #SELO - 17.2...: ===============================================================================================
							...
							$NUMEROPAPEL = '0';  
							$funcionario = $_SESSION['nome'];
							$insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','EDITAL_INTIMACAO','5','1','$SELO',CURRENT_TIMESTAMP,'CPL','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNO5')");
							$insert_auditoria->execute();
     ...
     for ($i=1; $i <=$busca_devedor->rowCount() ; $i++) {  
      #SELO - 17.2...: ===============================================================================================
							...
							$NUMEROPAPEL = '0';  
							$funcionario = $_SESSION['nome'];
							$insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','EDITAL_INTIMACAO','5','1','$SELO',CURRENT_TIMESTAMP,'CPL','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNO6')");
							$insert_auditoria->execute();
...
<div style="max-width: 100%; border-top: 1px dashed black"></div>
					<?php if (count($id)>1): ?>
