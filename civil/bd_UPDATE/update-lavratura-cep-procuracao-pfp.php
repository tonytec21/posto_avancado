...
header('location:../notas-lavratura-cep-procuracao-pfp.php?conf1=ok&id='.$id);
...
UPDATE_CAMPO_ID('notas_lavratura','setTipodeAtoCep',3,$id);
...
$_SESSION['sucesso'] = 'SUCESSO CONTINUE O PREENCHIMENTO DO FORMULÁRIO';
header('location:../notas-lavratura-cep-procuracao-pfp.php?conf4=ok&id='.$id);
...
$insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','PROCURAÇÃO P/ FINS PREV.','1','1','$seloExplode[$i]',CURRENT_TIMESTAMP,'$tipoSelo','$desTipoAto[$i]','$numLivro','$numFolha','$numOrdem','0','0','0')");
...
$pesquisa_livro = $pdo->prepare("SELECT * FROM livro_notas_procuracao_fins_previdenciarios where status = 'L' limit 1");
...
$pesquisa_livro_notas = $pdo->prepare("SELECT strLivro,strFolha,strFolhaFinal,strFolhaComplemento,setTipodeAtoCep,strSelo,quantidadeFolhas FROM `notas_lavratura` WHERE strFolha > 0 and setTipodeAtoCep = '3' and strLivro = '$livro'  and (strSelo IS NOT NULL || strSelo != '')   ORDER BY `notas_lavratura`.`strFolha` DESC LIMIT 1 ");
...
#Quando for cep:
$pesquisa_num_ordem = $pdo->prepare("SELECT strSelo,strFolha,strFolhaFinal,strFolhaComplemento,setTipodeAtoCep,Ordem, COUNT(Ordem) AS Quantidade FROM `notas_lavratura` WHERE strFolha > 0 and setTipodeAtoCep = '3' and (strSelo IS NOT NULL || strSelo != '') ORDER BY `notas_lavratura`.`strFolha`");
...
$pesquisa_num_ato = $pdo->prepare("SELECT strSelo,strFolha,strFolhaFinal,strFolhaComplemento,setTipodeAtoCep,numAto, COUNT(numAto) AS Quantidade FROM `notas_lavratura` WHERE strFolha > 0 and setTipodeAtoCep = '3' and (strSelo IS NOT NULL || strSelo != '') ORDER BY `notas_lavratura`.`strFolha`");
...
header('location:../notas-lavratura-cep-procuracao-pfp.php?conf3=ok&id='.$id);
