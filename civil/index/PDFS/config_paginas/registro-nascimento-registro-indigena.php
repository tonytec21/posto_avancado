...
<body>
	...
	if ($r->TIPOLOCALNASCIMENTO !='RANI') {
$_SESSION['erro'] = 'NÃO SERÁ POSSÍVEL EMITIR, REGISTRO NÃO FOI FEITO COMO REGISTRO INDIGENA!';
echo '<span style="color:white; background:red"> NÃO SERÁ POSSÍVEL EMITIR, REGISTRO NÃO FOI FEITO COMO REGISTRO INDIGENA!!</span>';
break;
}	
...
<?php endif ?></b>, na qualidade de
	...
	<?=$r->QUALIDADEDECLARANTE?>, e declarou que no dia <?php //echo date('d/m/Y', strtotime($r->dataObito));
...
<?php if ($r->DATANASCIMENTOMAE!=''): ?>
	...
	. São seus avós paternos: <b><?=strtoupper($r->AVO1PATERNO)?> e <?=strtoupper($r->AVO2PATERNO)?></b>. São seus avós maternos: <b><?=strtoupper($r->AVO1MATERNO)?> e <?=strtoupper($r->AVO2MATERNO)?></b>, <b>Nº DNV : <?=$r->DNV?></b>, <b>Nº RANI : <?=$r->RANI?></b> nada mais se declarou. Dou fé. Lido e achado conforme assina o declarante Eu 	<?=$_SESSION['nome']?> digitei subscrevo e assino.Selo de Fiscalização: <?=$r->SELO?>.
