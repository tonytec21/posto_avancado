				...
				<div class="container">
					...
					<legend class="bg-white col-lg-2" style="padding:20px;box-shadow: 0px 0px 10px 0px rgba(0, 0, 0, .2);">
						...
						<div class="col-lg-12 text-center">
								<h5 style="font-size:50%;"><?=$dados['NOMENASCIDO']?></h5>
								<p style="font-size:10px">Livro: <?=$dados['LIVRONASCIMENTO']?> <br> Folha: <?=$dados['FOLHANASCIMENTO']?> <br> Termo: <?=$dados['TERMONASCIMENTO']?> <br> Matricula: <?=$dados['MATRICULA']?> </p>
							</div> 
							<hr>

					<?php if (isset($_POST['TIPOCERTIDAO']) && $_POST['TIPOCERTIDAO'] == '6'):?>
						<div class="col-lg-12">
							<a href="pdfs/certidao-xml-nascimento.php?id=<?=$dados['ID']?>" class="btn btn-info btn-block">GERAR XML</a>
						</div>
					<?php endif ?>
					...
					<textarea class="tinymce-500" id="texto_nascimento" name="texto_nascimento">

						<?php if (isset($_GET['tiporegistro']) && $_GET['tiporegistro'] == '1'): ?>
							<?php include_once('pdfs/certidao-nascimento-primeiravia.php') ?>

						<?php elseif (isset($_POST['TIPOCERTIDAO']) && $_POST['TIPOCERTIDAO'] == '1'): ?>
							<?php include_once('pdfs/certidao-nascimento-primeiravia.php') ?>
						<?php elseif (isset($_POST['TIPOCERTIDAO']) && $_POST['TIPOCERTIDAO'] == '2'):?>
							<?php include_once('pdfs/certidao-nascimento-segundavia.php') ?>
						<?php elseif (isset($_POST['TIPOCERTIDAO']) && $_POST['TIPOCERTIDAO'] == '3'):?>
							<?php include_once('pdfs/certidao-nascimento-inteiroteor.php') ?>
						<?php elseif (isset($_POST['TIPOCERTIDAO']) && $_POST['TIPOCERTIDAO'] == '4'):?>
							<?php include_once('pdfs/certidao-nascimento-cancelamento.php') ?>	
						<?php elseif (isset($_POST['TIPOCERTIDAO']) && $_POST['TIPOCERTIDAO'] == '5'):?>
							<?php include_once('pdfs/certidao-nascimento-celibato.php') ?>	
						<?php elseif (isset($_POST['TIPOCERTIDAO']) && $_POST['TIPOCERTIDAO'] == '6'):?>
							<?php include_once('pdfs/certidao-nascimento-segundavia.php') ?>
						<?php elseif (isset($_POST['TIPOCERTIDAO']) && $_POST['TIPOCERTIDAO'] == '7'):?>
							<?php include_once('pdfs/certidao-nascimento-restauracao.php') ?>	
						<?php elseif (isset($_POST['TIPOCERTIDAO']) && $_POST['TIPOCERTIDAO'] == '8'):?>
							<?php include_once('pdfs/certidao-nascimento-portavel.php') ?>				
						<?php endif ?>
