...
if ($r->QUALIDADEDECLARANTE =='NULL' || $r->QUALIDADEDECLARANTE !='AMBOSPAI') {
$_SESSION['erro'] = 'NÃO SERÁ POSSÍVEL EMITIR, DADOS INFORMADOS NÃO COMPATÍVEIS COM ESTE TIPO DE ASSENTO!';
echo '<span style="color:white; background:red"> DADOS INFORMADOS NÃO COMPATÍVEIS COM ESTE TIPO DE ASSENTO!!</span>';
break;
}
...
<p style="text-align: justify;">Em	<?php //echo date('d/m/Y', strtotime($r->dataObito));
  ...
  if ($udg == '01' || $udg == '1' ||  $udg == '21'|| $udg == '31'|| $udg == '41' || $udg == '51'|| $udg == '61' || $udg == '71' || $udg == '81' || $udg == '91') {
   echo GExtenso::numero($novaData[0]).'  um';
	...
	<?php $w = PESQUISA_ALL('cadastroserventia');foreach ($w as $w):?><?=$w->strRazaoSocial?> Estado do Maranhão <?php endforeach ?>, no 2º Subdistrito do Ofício de Registro Civíl das Pessoas Naturais, nesta serventia, Compareceram os pais do registrado, e declaram que no dia 
...
<?php if ($r->DATANASCIMENTOMAE!=''): ?>
	 ...
	 <?php endif ?>
	São seus avós paternos: <b><?=strtoupper($r->AVO1PATERNO)?> e <?=strtoupper($r->AVO2PATERNO)?></b>. São seus avós maternos: <b><?=strtoupper($r->AVO1MATERNO)?> e <?=strtoupper($r->AVO2MATERNO)?></b>,
...
<?php endif ?>
	 <b>Nº DNV : <?=$r->DNV?></b>. Nada mais se declarou. Dou fé. Lido e achado conforme assina o declarante Eu 	<?=$_SESSION['nome']?> digitei subscrevo e assino.Selo de Fiscalização: <?=$r->SELO?>.
