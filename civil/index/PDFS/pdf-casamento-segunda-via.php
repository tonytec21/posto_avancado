...
elseif (empty($r->DATACASAMENTO)) {
$_SESSION['erro'] = "DATA DE CASAMENTO NÃO FOI PREENCHIDA!";
header('Location: ' . $_SERVER['HTTP_REFERER']);
exit(); 
}
...
include('../../selador/civil_geral.php');
      ...
      #PROXIMO PASSO ENVIANDO A SOLICITAÇÃO DO SELO: ===============================================================================================
                  ...
                  $nomeparte1 =tiraaspas($r->NOMECASADONUBENTE1);
                  $nomeparte2 =tiraaspas($r->NOMECASADONUBENTE2);
                ...
                $insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','2VIA_CASAMENTO','2','1','$SELO',CURRENT_TIMESTAMP,'CER','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNO')");
      ...
      if (isset($_GET['selobusca']) && $_GET['selobusca']!='') {
                            ...
                            $NUMEROPAPEL = $_GET['NUMEROPAPELSEGURANCA'];  
                            $funcionario = $_SESSION['nome'];
                            $insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','BUSCA_2VIA_CASAMENTO','2','1','$SELOBUSCA',CURRENT_TIMESTAMP,'CER','$ATO','$LIVRO','$FOLHA','$TERMO','$TIPOPAPEL','$NUMEROPAPEL','$RETORNOBUSCA')");
                            $UPSEGURANCA = $pdo->prepare("UPDATE folhaseguranca set STATUS = 'U' WHERE EMPRESA = '$TIPOPAPEL' AND PAPEL = '$NUMEROPAPEL'");
                            $UPSEGURANCA->execute();  
...
<?php if (isset($SELO)) {include('config_paginas/registro-casamento-segunda-via.php'); } ?>
...
<br><br>  
<a  class="col-md-12 btn waves-effect bg-orange" href="../../bd_INSERTS/gerador-certidao-xml-casamento.php?id=<?=$id?>&CODIGOPEDIDOXML=<?=$_GET['CODIGOPEDIDOXML']?>">GERAR ARQUIVO XML</a>
