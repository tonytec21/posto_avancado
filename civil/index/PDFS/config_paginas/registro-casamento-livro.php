...
if (isset($_GET['id_apagar'])) {$id_apagar = $_GET['id_apagar'];
    $del = $pdo->prepare("DELETE FROM salvar_editor where IDREGISTRO ='$id_apagar' and TIPO = 'TERMO_CASAMENTO' ");
    $del->execute();
    }
...
body{ font-size: 12px;font-family: "Arial";margin-left: 1.5cm;margin-bottom: -10cm;
...
if (isset($_GET['preview']) && $r->ANDAMENTOPROCESSO == 'PROCLAMAS') {
...
<?php if ($r->TIPOLIVRO=='3' ): ?>
	<span style="color: red">CASAMENTO NÃO FOI PREENCIDO COMO LIVRO B.</span>
<?php break; endif ?>
...
<!--CABEÇAÇHO ------------------------------------------------------------------------------------------------>
			...
			<h3 style="display: inline;margin-left: 0cm">LIVRO: E <?=intval($r->LIVROCASAMENTO)?></h3>
			<?php else: ?>		
			<h3 style="display: inline;margin-left: 0cm">LIVRO: B <?=intval($r->LIVROCASAMENTO)?></h3>
			<?php endif ?>	
			...
			?> (<?=date('d/m/Y', strtotime($data))?>), no(a) <?=$r->LOCALCERIMONIA?>
			às <?=$r->HORACASAMENTO?> horas perante o(a) Sr(a).
			<?=$r->JUIZPAZ?>, Juiz(a) de Casamentos em Exercício, comigo, oficial de
			seu cargo, no fim nomeado e assinado na presença de testemunhas, receberam-se em matrimônio os contraentes: 

			<p style="font-weight: bold; text-align: center"><?=$r->NOMENUBENTE1?> e <?=$r->NOMENUBENTE2?></p>
		<!--NUBENTE1 -------------------------------------------------------------------------------------------------------->	
			<p style="text-align: justify;">O(a) Conjuge 
					...
					natural de <?php  $c = PESQUISA_ALL_ID('uf_cidade',intval($r->CIDADEPAINUBENTE1)); foreach($c as $c):?>
					<?php if (intval($r->CIDADEPAINUBENTE1) == "5300109"): ?>
					<span style="text-transform: capitalize;"><?=str_replace($tirar_cidades,$repor_cidades,$r->CIDADEPAINUBENTE1)?></span>
					<?php else: ?>
					<span style="text-transform: capitalize;"><?=mb_strtolower($c->cidade)?></span>/<?=$c->uf?>,
					<?php endif ?>	
					<?php endforeach ?>
					...
					natural de <?php  $c = PESQUISA_ALL_ID('uf_cidade',intval($r->CIDADEMAENUBENTE1)); foreach($c as $c):?>
					<?php if (intval($r->CIDADEMAENUBENTE1) == "5300109"): ?>
					<span style="text-transform: capitalize;"><?=str_replace($tirar_cidades,$repor_cidades,$r->CIDADEMAENUBENTE1)?></span>
					<?php else: ?>
					<span style="text-transform: capitalize;"><?=mb_strtolower($c->cidade)?></span>/<?=$c->uf?>,
					<?php endif ?>	
					<?php endforeach ?>
		...
		<!--NUBENTE2 -------------------------------------------------------------------------------------------------------->	
			<p style="text-align: justify;">O(a) Conjuge 
					...
					natural de <?php  $c = PESQUISA_ALL_ID('uf_cidade',intval($r->CIDADEPAINUBENTE2)); foreach($c as $c):?>
					<?php if (intval($r->CIDADEPAINUBENTE2) == "5300109"): ?>
					<span style="text-transform: capitalize;"><?=str_replace($tirar_cidades,$repor_cidades,$r->CIDADEPAINUBENTE2)?></span>
					<?php else: ?>
					<span style="text-transform: capitalize;"><?=mb_strtolower($c->cidade)?></span>/<?=$c->uf?>,
					<?php endif ?>	
					<?php endforeach ?>
					...
					natural de <?php  $c = PESQUISA_ALL_ID('uf_cidade',intval($r->CIDADEMAENUBENTE2)); foreach($c as $c):?>
					<?php if (intval($r->CIDADEMAENUBENTE2) == "5300109"): ?>
					<span style="text-transform: capitalize;"><?=str_replace($tirar_cidades,$repor_cidades,$r->CIDADEMAENUBENTE2)?></span>
					<?php else: ?>
					<span style="text-transform: capitalize;"><?=mb_strtolower($c->cidade)?></span>/<?=$c->uf?>,
					<?php endif ?>	
					<?php endforeach ?>
		...
		<!--TEXTO DO CASAMENTO ----------------------------------------------------------------------------------------------->
							...
							casarem e (IV) Declaração dos contraentes. E para constar, lavrei este termo, que sendo lido e
							achado conforme, vai devidamente assinado pelo Juiz de Paz, contraentes e testemunhas.

							<?php if ($r->ATOCASAMENTO =='14.3.3'): ?>
							...
							<?php endif ?>
							 ...
							 Eu,
							<?=mb_strtoupper($nome_assinatura)?>, <?=mb_strtoupper($funcao_assinatura)?> dou fé e assino ______________________________________. 	 
							Selo de Fiscalização: <?=strip_tags($r->SELO)?>. Matrícula da 1ª Via da Certidão: <?=$r->MATRICULA?> ---------------------------------------- <span style="font-size: 8px;">Documento impresso por meio eletrônico. Qualquer rasura ou indício de adulteração será considerado fraude.</span>
		...
		<!--TEXTO DO CASAMENTO ----------------------------------------------------------------------------------------------->
				...
				<p style="text-align: center">_____________________________________</p> 
						<p style="text-align: center;margin-top: -10px;"><?=strtoupper($r->JUIZPAZ)?></p> 
						<p style="text-align: center;margin-top: -10px;">JUIZ DE PAZ</p> 	
					...
					<?php endif ?>
						...
						<?php if ($r->ROGOTESTEMUNHA1!=''): ?>
						<p >
						<br>		
						<div style="border: 3px dashed silver; width: 5%; height: 30px; padding: 30px;margin-left: 20%;margin-bottom: -5%"></div>
						<p style="display: inline; margin-left: 40%;">________________________________________________</p><br>
						<p style="display: inline;margin-left: 22%;font-size: 8px;">(Digital TESTEMUNHA1)</p>
						<p style="display: inline;margin-left: 22%;font-size: 8px;">&nbsp&nbsp&nbsp<?=$r->ROGOTESTEMUNHA1?></p>
						</p>	
						<?php else: ?>
							
						<p style="max-width:  100%; text-align: center;margin-top:-10px;">__________________________________________________ <br> <?=strtoupper($r->NOMETESTEMUNHA1)?></p>
						
						<?php endif ?>

						<?php if ($r->ROGOTESTEMUNHA2!=''): ?>
						<p >
						<br>		
						<div style="border: 3px dashed silver; width: 5%; height: 30px; padding: 30px;margin-left: 20%;margin-bottom: -5%"></div>
						<p style="display: inline; margin-left: 40%;">________________________________________________</p><br>
						<p style="display: inline;margin-left: 22%;font-size: 8px;">(Digital TESTEMUNHA2)</p>
						<p style="display: inline;margin-left: 22%;font-size: 8px;">&nbsp&nbsp&nbsp<?=$r->ROGOTESTEMUNHA2?></p>
						</p>	
						<?php else: ?>
							
						<p style="max-width:  100%; text-align: center;margin-top:-10px;">__________________________________________________ <br> <?=strtoupper($r->NOMETESTEMUNHA2)?></p>
...
<td style="border:none;max-width: 10%;">
	<img style="width: 100px;" src="<?=$nome?>" />
</td>
<td style="border:none; width:80%; text-align:justify;font-size:8px">
	<?=$textoselo?>
</td>		
...
foreach ($ba as $ba) {
echo '<h4 style="">Anotações/Averbações</h4><br> <p style="max-width: 699px;word-wrap: break-word;text-align: justify;">'.$ba->strAverbacao;
}
...
foreach ($ban as $ban) {
echo '<h4 style="">Anotações/Averbações</h4><br> <p style="max-width: 699px;word-wrap: break-word;text-align: justify;">'.$ban->ANOTACAO;
}
...
if ($r->AVERBACAOTERMOANTIGO!='') {
	echo '<h4 style="">Anotações/Averbações</h4><br> <p style="max-width: 699px;word-wrap: break-word;text-align: justify;">'.$r->AVERBACAOTERMOANTIGO;
}
 ?>
<p style="text-align: justify;margin-top: 130px"> 				
<?php  
