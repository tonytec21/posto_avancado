        ...
        <section class="content" style="margin-left: 30px"  >
              ...
              <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                           <h2 class="col-md-7">REGISTRO OBITO LIVRO C</h2>
                           ...
                           <a href="pdf-obito-livro-auxiliar?id=<?=$id?>&id_apagar=<?=$id?>" class="btn col-md-3 waves-effect bg-red "><i class="material-icons">stop_screen_share</i> Limpar edições feitas </a>
...
if (isset($_GET['v2'])) {
   include('config_paginas/registro-obito-livro-auxiliarv2.php');
}
elseif (isset($_GET['v3'])) {
   include('config_paginas/registro-obito-livro-auxiliarv3.php');
}
else{
 include('config_paginas/registro-obito-livro-auxiliar.php');
} 
 ?>
...
$up = $pdo->prepare("UPDATE salvar_editor set TEXTO = '$texto_salvar' where IDREGISTRO = '$id' and TIPO = 'TERMO_OBITO_AUXILIAR' ");
$up->execute();
}
else{
$in = $pdo->prepare("INSERT into salvar_editor values('$id', 'TERMO_OBITO_AUXILIAR', '$texto_salvar')");
$in->execute();
}
...
$_SESSION['sucesso'] = 'ALTERAÇÕES SALVAS';
 ...
 ?>
  ...
  <script type="text/javascript">
    window.location.assign('pdf-obito-livro-auxiliar.php?id=<?=$id?>');
   ...
   <script type="text/javascript">
    ...
    $('#versaotermo').change(function(){
      if ($('#versaotermo').val() == '1') {
        window.location='pdf-obito-livro-auxiliar.php?id=<?=$id?>';
      }
      else if ($(this).val() == '2') {
          window.location='pdf-obito-livro-auxiliar.php?id=<?=$id?>&v2=ok';
        }

      else if ($(this).val() == '3') {
          window.location='pdf-obito-livro-auxiliar.php?id=<?=$id?>&v3=ok';
        }
    });
