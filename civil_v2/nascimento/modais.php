    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="col-lg-12">
          ...
          <button class="btn btn-info btn-lg btn-block" onclick="gerarmatricula($('#LIVRONASCIMENTO').val(), $('#FOLHANASCIMENTO').val(), $('#TERMONASCIMENTO').val(), 'NA', '1', $('#TIPOACERVO').val())"> GERAR MATRICULA</button>
        </div>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <hr>
        <div class="row">
          <div class="col-lg-6"><label for="country">ATO</label><input class="form-control" type="text" name="ATONASCIMENTO" id="ATONASCIMENTO" placeholder="ATO ex: 14.11" list="atosliberados"></div>
          ...
          <datalist id="atosliberados">
            ...
            <option value="14.3.4">14.3.4 - Procedimento de adoção e reconhecimento de filho por determinação judicial, excluída a certidão.
          ...
          <div class="col-lg-12"><label for="country"></label><button id="botaoseloregistro" class="btn btn-info btn-lg btn-block"
            onclick="solicitarselo('<?=$id?>',$('#LIVRONASCIMENTO').val(),$('#FOLHANASCIMENTO').val(),$('#TERMONASCIMENTO').val(),$('#NOMENASCIDO').val(),$('#MOTIVOSELOISENTO').val(),$('#ATONASCIMENTO').val(),$('#QUANTIDADE').val(),$('#TIPOPAPELSEGURANCA').val(), $('#NUMEROPAPELSEGURANCA').val(), 'REGISTRO_NASCIMENTO')" 
          ...
          <div class="row">
            ...
            <div class="col-lg-12">
              <a href="info-registro-nascimento.php?id=<?=$id?>" id="visualizarregistro" class="btn btn-block btn-success">IR PARA REGISTRO</a>
            </div>


            <div class="card col-lg-12" id="contselos" style="padding-top:40px">
              <?=montarselo($id, "REGISTRO_NASCIMENTO")?>
            </div>  


            <?php if (isset($_SESSION['logadoAdm']) && $_SESSION['logadoAdm'] == 'S'): ?>                
              <div class="col-lg-4"><label for="country"></label><button onclick="solicitarselo('<?=$id?>',$('#LIVRONASCIMENTO').val(),$('#FOLHANASCIMENTO').val(),$('#TERMONASCIMENTO').val(),$('#NOMENASCIDO').val(),$('#MOTIVOSELOISENTO').val(),$('#ATONASCIMENTO').val(),$('#QUANTIDADE').val(),$('#TIPOPAPELSEGURANCA').val(), $('#NUMEROPAPELSEGURANCA').val(), 'REGISTRO_NASCIMENTO')" class="btn btn-danger btn-lg btn-block"><i class="fa fa-reply"></i> SELAR NOVAMENTE</button>  <br><br></div>
            <?php endif ?>     
        </div>
      </div>
      <div class="modal-footer">
