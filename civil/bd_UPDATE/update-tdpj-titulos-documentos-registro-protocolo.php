...
$pesquisa_livro = $pdo->prepare("SELECT * FROM livro_tdpj_td_a where status = 'L' limit 1");
...
$pesquisa_num_ordem = $pdo->prepare("SELECT strFolha,strFolhaFinal,strFolhaComplemento,selecionarLivro,Ordem, COUNT(Ordem) AS Quantidade FROM `tdpj_td_registro_protocolo` where strSelo is not null and strSelo != ''   ORDER BY `tdpj_td_registro_protocolo`.`Ordem`");
...
$pesquisa_num_ato = $pdo->prepare("SELECT strFolha,strFolhaFinal,strFolhaComplemento,selecionarLivro,numAto, COUNT(numAto) AS Quantidade FROM `tdpj_td_registro_protocolo` where strSelo is not null and strSelo != ''   ORDER BY `tdpj_td_registro_protocolo`.`numAto`");
...
$registro_rotocolo = $pdo->prepare("INSERT INTO `tdpj_td_registro_protocolo` (`dataAto`,`strLivro`,`Ordem`,`numAto`,`strSelo`,`strDescricaoAto`,`strNomeParte1`,`strCPFParte1`,`setTipoDocumento2Parte1`,`strNaturezaTitulo`,`strQualidadeLancamento`,`strAnotacoes`) VALUES ('$dataAto','$livro','$numOrdem','$numAto','$strSelo','$strDescricaoAto','$strNomeParte1','$strCPFParte1','$setTipoDocumento2Parte1','$strNaturezaTitulo','$strQualidadeLancamento','$strAnotacoes');");
if ($registro_rotocolo->execute()) {

}else{
	$linhas = 'Erro!!!';
	echo $linhas;
	echo $registro_rotocolo->errorInfo();
	return false;
	break;
}

$funcionario = $_SESSION['nome'];
$insert_auditoria = $pdo->prepare("INSERT into auditoria values (NULL,'$funcionario','PROTOCOLO - TD','1','1','$selo',CURRENT_TIMESTAMP,'GER','$desTipoAtoGER','0','0','$numOrdem','0','0',NULL)");
$insert_auditoria->execute();

$busca_procolo = $pdo->prepare("SELECT * FROM `tdpj_td_registro_protocolo` ORDER BY ID DESC LIMIT 1");
$busca_procolo->execute();
$linhas_protocolo = $busca_procolo->fetch(PDO::FETCH_ASSOC);
$id_protocolo = $linhas_protocolo['ID'];
$registro_pj = $pdo->prepare("INSERT INTO `tdpj_td_registro` (`possuiProtocolo`,`Ordem`,`numAto`,`decValorInfoImovel`) VALUES ('0','','','');");
$registro_pj->execute();
$busca_registro_pj = $pdo->prepare("SELECT * FROM `tdpj_td_registro` ORDER BY ID DESC LIMIT 1");
$busca_registro_pj->execute();
$linhas_busca_pj = $busca_registro_pj->fetch(PDO::FETCH_ASSOC);
$id = $linhas_busca_pj['ID'];

UPDATE_CAMPO_ID('tdpj_td_registro_protocolo','IDregistro',$id,$id_protocolo);


UPDATE_CAMPO_ID('tdpj_td_registro','strNumProtocolo',$numOrdem,$id);
UPDATE_CAMPO_ID('tdpj_td_registro','strNaturezaTitulo',$_POST['strQualidadeLancamento'],$id);
...
header('location:../imprimir/impressao-td-protocolo.php?id='.$id_protocolo);
